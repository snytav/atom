...
2: each parameter line must follow the structure "numeric_parameter			# comment about numeric_parameter"
	...
	2.2: comment about parameter can be empty, but '#' character is always needed
...
1: open system console and execute command "python plasma_init_code.py input_config.txt output_plasma.nc [-d|--display]"; where input_config.txt is the text file containing plasma parameters, output_plasma.nc will be the generated NetCDF file, and where "-d" or "--display" are options to display 3D view of the generated plasma at the end
